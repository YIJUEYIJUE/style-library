(function(){
  ...
  // restrained accent palette (category marks only)
  const PAL = ['#2a52cc','#c2533f','#1f9282','#b8860b','#7a4fb0','#3a8f5a'];
  const colorMap = {};
  cats.forEach((c,i)=>{ colorMap[c.key] = PAL[i % PAL.length]; });
  ...
  function buildCats(){
    ...
    const all = [{k:'ALL',n:'全部风格',ci:-1}].concat(
      cats.map((c,i)=>({k:c.key,n:c.name,ci:i}))
    );
    $('#catList').innerHTML = all.map((c,idx)=>{
      const num = c.k==='ALL' ? '·' : String(idx).padStart(2,'0');
      const col = c.ci<0 ? 'var(--ink)' : PAL[c.ci % PAL.length];
      return `<button class="cat ${state.cat===c.k?'active':''}" data-cat="${esc(c.k)}" style="--c:${col}">
        <span class="catNo">${num}</span>
        <b>${esc(c.n)}</b>
        <i>${counts[c.k]||0}</i>
      </button>`;
    }).join('');
  ...
  function renderGrid(){
    ...
    $('#listKicker').textContent = catName;
    ...
    $('#grid').innerHTML = list.map((it,idx)=>{
      ...
      const dot = colorMap[it.catKey] || 'var(--blue)';
      return `<button class="card" data-id="${id}">
        ${img}
        <div class="cardTitle">${esc(it.title.slice(0,40))}</div>
        <div class="cardMeta">
          <span class="cardCat"><i class="dot" style="background:${dot}"></i>${esc(catName)}</span>
  ...
  function openModal(id){
    ...
    cur = it; gi = 0;
    const dot = colorMap[it.catKey] || 'var(--blue)';
    $('#mCat').innerHTML = `<i class="dot" style="background:${dot}"></i>${esc(it.catName)}`;
